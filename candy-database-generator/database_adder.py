...
def extract_product_data(text):
    ...
    categoryModel = lines[3].strip()
    return price, categoryModel, name, description
...
def write_to_csv(data, filename):
    ...
    with open(filename, 'a', newline='') as csvfile:
        ...
        if not file_exists:
            writer.writerow(['price', 'image_uri','categoryModel','view_count','is_favourite','cart_quantity','name', 'description'])
        writer.writerow(data)
    print(f"CSV file '{filename}' generated/edited successfully.")

while True:
    # Take input from the user
    input_text = input("Enter the product details (name / price / description / categoryModel) seperated by a forward slash:\n")
